...
func (s *Socket) EmitAck(call func(uint64, json.JsonArr), event string, objs ...any) (err error) {
	if call == nil {
		panic("call == nil")
	}
	err = s.send(&SPacket{typ: SP_EVENT, id: s.ackid, data: (json.JsonArr)(append([]any{event}, objs...))})
	if err != nil {
		return
	}
