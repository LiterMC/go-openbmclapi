...
type threadPool[S any, T any] struct {
	limit   int
	running int
	notify  *sync.Cond
	result  []T
...
func newThreadPool[S any, T any](limit int) *threadPool[S, T] {
	return &threadPool[S, T]{
		limit:  limit,
		notify: sync.NewCond(new(sync.Mutex)),
		result: make([]T, 0, 8),
		doneCh: make(chan struct{}, 0),
	}
}

func (p *threadPool[S, T]) Release() {
	p.notify.L.Lock()
	if !p.done {
		p.done = true
		close(p.doneCh)
...
func (p *threadPool[S, T]) Do(fn func(S) T, arg S) {
	for {
		p.notify.L.Lock()
		if p.done {
			return
		}
		if p.running+1 < p.limit {
			p.running++
			p.notify.L.Unlock()
			break
		}
...
func (p *threadPool[S, T]) Wait() <-chan T {
	resultCh := make(chan T, 0)
	go func() {
		defer close(resultCh)
		var results []T
		for {
			p.notify.L.Lock()
			if p.done || p.running > 0 {
				return
			}
			for {
				results, p.result = p.result, results[:0]
				if len(results) > 0 {
					break
				}
				p.notify.Wait()
